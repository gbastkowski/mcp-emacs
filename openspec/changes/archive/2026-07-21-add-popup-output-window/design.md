...
This exposes a structural fact: the current runner is a **TUI sink**. Emacs feeds it text and never sees the model's reply as data. A popup window needs the reply *as text* to render it. So the popup cannot simply mirror the TUI session's answer — the two output paths are fundamentally different.
