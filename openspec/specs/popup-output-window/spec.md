...
Render ephemeral, formatted AI output in a reusable popup window near the code
the user is looking at, instead of only feeding it into the runner session
terminal. Content is markdown, displayed read-only with syntax-highlighted code,
in an ordinary split window that the user can scroll, select, and copy from.

## Requirements

### Requirement: Render markdown content in a popup output window
...
### Requirement: Support scroll, select, and copy in the output window
The buffer shown by the popup output window SHALL behave as a normal Emacs buffer, allowing the user to scroll it, set a region, and copy text from it.
