...
Define the MCP code-diagnostics tools: a buffer-scoped tool
(`get_buffer_diagnostics`) and a project-scoped tool
(`get_project_diagnostics`), distinct from Emacs-error investigation
(`get_error_context`) and Emacs-health tooling (`diagnose_emacs`).
