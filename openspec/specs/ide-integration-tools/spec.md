...
Define harness-agnostic, pull-model MCP tools that bring editor-integration features into `mcp-emacs` over the existing HTTP transport: an interactive `ediff`-based diff/apply flow and editor-state queries (open editors, document dirty state), without depending on any client-specific IDE protocol.
