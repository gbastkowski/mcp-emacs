...
Let a running Emacs act as the Claude Code IDE for a workspace so that Claude Code's native file edits are reviewed through Emacs' interactive ediff flow before being written. This capability speaks Claude Code's client-specific IDE protocol over WebSocket (JSON-RPC 2.0), launches Claude Code with the IDE integration environment, and publishes the discovery lockfile Claude Code expects. It is opt-in and disabled by default, and coexists with the existing HTTP MCP server without altering it.
