...
Define the MCP capability that lets a caller block until an Org task session file changes relative to a baseline change token, or until a bounded timeout elapses, without blocking the Emacs event loop, and returns the current session view on wake.
